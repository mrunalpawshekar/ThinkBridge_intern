...
class english:	
	def numToWords(self,n, s):  # n is 1 or 2 digit number and s if for 'thousand' , 'hundred', 'lakh' , crore
		str2=""
		if (n > 19):        
	...
	def convertToEnglish(self,n): 
		output =" " 			# Function to print a given number in words 
		output += self.numToWords((n // 10000000),"crore ")    # for ten millions and hundred millions places
		output +=self.numToWords(((n // 100000) % 100),"lakh ")   # for hundred thousands  and one millions places 
		output +=self.numToWords(((n // 1000) % 100),"thousand ") #for  thousands and tens thousands places  
		output += self.numToWords(((n // 100) % 10), "hundred ")   # hfor hundreds places 
		output += self.numToWords((n % 100), "")  # handles digits at ones and tens places
		return output

class Hindi:
	def converttohindi(self,n): 
		
		output =""
		output += self.numbers((n // 10000000),"करोड़  ")    # for ten millions and hundred millions places
		output += self.numbers(((n // 100000) % 100)," लाख ")   # for hundred thousands  and one millions places 
		output += self.numbers(((n // 1000) % 100)," हज़ार") #for  thousands and tens thousands places  
		output += self.numbers(((n // 100) % 10), " सौ ")   # hfor hundreds places 
		output += self.numbers((n % 100), "")  # handles digits at ones and tens places
		return output

	def numbers(self,n,s):
		str2 = " "
		if ( n <= 100 ):
			str2 += listhindi[n]
		elif (n == 0):
			...
			str2 += self.converttohindi(n)
		if (n):
			str2 += s
		return str2
	
class Marathi:
	def converttomarathi(self,n): 
		output =""
		output += self.numbers((n // 10000000),"कोटी  ")    # for ten millions and hundred millions places
		output += self.numbers(((n // 100000) % 100)," लाख ")
		output += self.numbers(((n // 1000) % 100)," हजार") #for  thousands and tens thousands places  
		output += self.numbers(((n // 100) % 10), "शे")   # hfor hundreds places 
		output += self.numbers((n % 100), "")  # handles digits at ones and tens places
		return output
	...
	def numbers(self,n,s):
		...
		if ( n <= 100 ):
			...
			str2 += self.converttomarathi(n)
		if (n):
			str2 += s
		return str2

def main():
	ej=english()    #objects for each language class
	hi=Hindi()
	mr=Marathi()
	value=float(input(" \n\n enter amount:"))
	n=int(value)
	...
	while(1):
		...
		if(choice == '1'):
			print("\n\n", ej.convertToEnglish(n),decimal, "/100","\n\n")
		if(choice == '2'):
			print("\n\n", hi.numbers(n,''),decimal,"/100","\n\n")
		if(choice == '3'):
			print("\n\n" ,mr.numbers(n,''),decimal,"/100","\n\n")
		if(choice == '4'):
			exit()
